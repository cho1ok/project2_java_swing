...
, board_id NUMBER
, rtitle varchar2(200)
, rwriter varchar2(200)
, rregdate DATE DEFAULT sysdate
, CONSTRAINT fk_board_reply FOREIGN KEY (board_id) references board(board_id)
);
...
SELECT b.board_id AS board_id, title, writer, count(r.reply_idx) AS re
 FROM board b LEFT OUTER join reply r
 on b.board_id=r.board_id
 GROUP BY b.board_id, title, writer;
 
INSERT INTO board(board_id, title, writer, content)
 values(seq_board.nextval, 'today', 'hj', 'nothing');
 
INSERT INTO reply(reply_idx, board_id, rtitle, rwriter)
 values(seq_reply.nextval, 1, 'babo', 'qq');